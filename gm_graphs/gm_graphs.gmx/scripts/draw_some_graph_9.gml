...
Paramètre supplémentaire : boucle for en SIMD vs avec un for classique.
...
for (var ij = 0; ij < ds_list_size(ctrl.jobs_fixed_list); ++ij) {
    ...
    for (var ids = 0; ids < ds_list_size(j.datasets); ++ids) {
        ...
        //"dataset " + split_thousands(ids)
            ...
            if (j.SIMD_FOR_LOOP == 1) { // copy strategy SYCL or mem on device (no glibc on device)
                memcopy_name = "for SIMD";
                memcopy_short_name = "s";
            } else {
                memcopy_name = "for classique";
                memcopy_short_name = "c";
            }

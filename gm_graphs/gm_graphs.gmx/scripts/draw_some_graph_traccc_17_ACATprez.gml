...
for (var loop_ij = 0; loop_ij < ds_list_size(ctrl.jobs_fixed_list); ++loop_ij) {
    ...
    for (var ids = 0; ids < ds_list_size(j.datasets); ++ids) {
        ...
        if (lsize != 0) {
            ...
            var gpshort_name = "NC";
            var gpname = "inconnu";
            if (j.MEMORY_STRATEGY == 1) {
                gpshort_name = mem_location_to_str_prefix(j.MEMORY_LOCATION) + "_pt";
                gpname = mem_location_to_str(j.MEMORY_LOCATION) + ", pointer graph (" + gpshort_name + ")";
            }
            ...
            for (var i_iteration = 0; i_iteration < lsize; ++i_iteration) {
                ...
                var as_x = 0 + gxoffset;
                var as_y = iter.t_alloc_only; //; t_flatten_alloc
                if (g_split_thousands_USE_MS) as_y = round(as_y / 1000) + 1;
                var pt = instance_create(0, 0, graph_single_point);
                // iter.t_alloc_only et iter.t_fill_only 
                pt.xlabel = "SYCL alloc";
                pt.xx = as_x;
                pt.ylabel = split_thousands(as_y);
                if (echelle_log) pt.yy = log2(as_y);
                else             pt.yy = as_y;
                pt.color = gp.color;
                ds_list_add(gp.points, pt);
                ...
                var as_y = iter.t_fill_only; //iter.; t_flatten_fill
                if (g_split_thousands_USE_MS) as_y = round(as_y / 1000) + 1;
                var pt = instance_create(0, 0, graph_single_point);
                // iter.t_alloc_only et iter.t_fill_only 
                ...
                // Pas super intéressant la lecture
                /*
...
var ltext;
if (g_split_thousands_USE_MS) ltext = "Elapsed time ms";
else                          ltext = "Elapsed time µs";

draw_graph_objs(graph_list, 20, 20, "", ltext, 0, g_ymax_impose, -1, -1);

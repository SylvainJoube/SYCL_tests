...
enum sycl_mode {implicit_USM, device_USM, host_USM, accessors};
...
void main_sequence(
    ...
    void (*sycl_allocation)(cl::sycl::queue &, host_dataset *, gpu_timer &),
    void (*sycl_compute)(cl::sycl::queue &, host_dataset *, gpu_timer &),
    void (*sycl_free)(cl::sycl::queue &, host_dataset *, gpu_timer &)
    ) {
    ...
    try {
        ...
        for (int ids = 0; ids < DATASET_NUMBER; ++ids) {
            ...
            for (int rpt = 0; rpt < REPEAT_COUNT_REALLOC; ++rpt) {
                ...
                sycl_allocation(sycl_q, dataset, gtimer);
                sycl_compute(sycl_q, dataset, gtimer);
                sycl_free(sycl_q, dataset, gtimer);
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    main_sequence(myfile, explicit_USM_allocation, explicit_USM_compute, explicit_USM_free);

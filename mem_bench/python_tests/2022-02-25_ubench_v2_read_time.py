...
global_drawn_x_variables_number = 7 + global_kernel_retain
...
for header in header_list:
    ...
    if found:
        ...
        for iteration in header["iterations"]:
            ...
            y_list[ik].append(nz(iteration["t_read"])           / divide_by)
            y_list[ik + 1].append(nz(iteration["t_dealloc_sycl"])       / divide_by)
            y_list[ik + 2].append(nz(iteration["t_dealloc_native"])     / divide_by)
...
#     plt.title("Microbenchmark - accessors & USM device & shared/host")
...
def draw_tab():
    ...
    if tableau_horizontal:
        ...
        if (FLAT_ONLY):
            print("& native-a & sycl-a & fill & copy & ker\\textsubscript{1} & ker\\textsubscript{2} & read & sycl-d & native-d & total & \\\\ [0.5ex]")
            print("\\hline\\hline")
            print(draw_tab_item("device", "dev", y_median_device))
            print("\\hline")
            print(draw_tab_item("accessors", "acc", y_median_acc))
            print("\\hline")
            print(draw_tab_item("shared", "sha", y_median_shared_direct))
            print("\\hline")
            print(draw_tab_item("host", "hos", y_median_host_direct))
